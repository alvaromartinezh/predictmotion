"""Monte Carlo de la fase de grupos del Mundial — port de simulateGroups().

Ajusta las probabilidades de cada partido por ranking FIFA (Elo derivado),
exactamente como mundial.html. Devuelve por selección: prob de ser 1ª/2ª/3ª de
grupo, prob de quedar eliminada y prob de pasar a la fase eliminatoria (pAdv).
"""

import math

from .config import SIM_N_CUP
from .prng import make_rng, groups_seed
...
def _match_probs(a, b):
    """[pWin, pDraw, pLoss] para a vs b, ajustado por ranking. Port exacto."""
    diff = _elo(a) - _elo(b)
...
def simulate(groups, sim_n=SIM_N_CUP):
