/* PREVIEW de assets/home.js — copia aislada para probar el carrusel de
 * "Artículos" en el feed personalizado sin tocar la home real. Ver plan en
 * /home/cachito/.claude/plans/shimmering-stargazing-parrot.md. Todo lo
 * marcado "NUEVO" es lo único que difiere de home.js; el resto es una
 * copia literal (misma lógica, mismos nombres) para que graduar el delta
 * a home.js sea un diff limpio.
 *
...
(function () {
  ...
  var ARTICLE_TYPE_LABEL = {
    recap_jornada: 'Recap', explicador_probabilidad: 'Explicador', carrera_titulo: 'Carrera',
  ...
  function articleCarousel(items) {
    if (!items || !items.length) return '';
    return '<div class="acarousel-wrap"><div class="acarousel">' + items.map(articleCard).join('') + '</div>'
      + '<button class="acarousel-nav" type="button" aria-label="Ver más artículos">›</button></div>';
  }
  function wireArticleCarousel() {
    document.querySelectorAll('.acarousel-wrap').forEach(function (w) {
      var c = w.querySelector('.acarousel'), btn = w.querySelector('.acarousel-nav');
      function sync() {
        var atEnd = c.scrollWidth <= c.clientWidth + 4 || c.scrollLeft + c.clientWidth >= c.scrollWidth - 4;
        w.classList.toggle('at-end', atEnd);
      }
      if (btn) btn.addEventListener('click', function () { c.scrollBy({ left: c.clientWidth * 0.8, behavior: 'smooth' }); });
  ...
  // de noticias de lo que sigue — sin más tablas ni partidos. NUEVO (preview): rail
  // de "Artículos" filtrado por follows, después de "Más noticias".
  function buildUserHTML(f, matches, snaps, allNews, tables, liveMatchDetail, allArticles) {
    ...
    // NUEVO: rail "Artículos" — mismo predicado leagues/teams que las noticias de
    // arriba, sección propia (no se mezcla en el rail de noticias), se omite si no
    // hay nada relevante (mismo criterio que el resto de secciones del feed).
    var relArticles = (allArticles || []).filter(function (a) {
      return (a.leagues || []).some(function (l) { return fLeagues[l]; }) || (a.teams || []).some(function (t) { return fTeamIds[String(t.id)]; });
    });
    if (relArticles.length) {
      col.push(feedSec('Artículos', 'Ver todos', '/articulos'));
      col.push(articleCarousel(relArticles.slice(0, 10)));
    }
  ...
  // ── feed anónimo ── NUEVO (preview): rail "Artículos" sin filtrar (más
  // recientes de todas las ligas), entre "Predicciones que suenan" y "Lo último".
  function buildAnon() {
    var today = new Date(); var ymd = today.getFullYear() + ('0' + (today.getMonth() + 1)).slice(-2) + ('0' + today.getDate()).slice(-2);
    return Promise.all([D.scoreboard('laliga', ymd), D.snapshot('laliga'), D.news(), D.liveTable('laliga'), fetchArticles()]).then(function (res) {
      var events = (res[0] || []).map(function (e) { return D.parseEvent(e, 'laliga'); }).filter(Boolean);
      var snap = res[1], allNews = res[2] || [], table = res[3], allArticles = res[4] || [], col = [];
      ...
      if (allArticles.length) { col.push(feedSec('Artículos', 'Ver todos', '/articulos')); col.push(articleCarousel(allArticles.slice(0, 10))); }
  ...
  // NUEVO: onRender:wireArticleCarousel — home.js real no necesita onRender
  // (todas sus tarjetas son enlaces puros); el carrusel es la primera pieza con
  // comportamiento propio (scroll-snap + botón), hay que re-engancharlo tras
  // cada innerHTML swap de PMShell.mount, mismo patrón que partidos.js/search.js.
  function mount(out) { window.PMShell.mount({ active: 'home', main: shellMain(out.col, out.rail), onRender: wireArticleCarousel }); }
  ...
  function refresh() {
    ...
    Promise.all([D.news(), primary ? D.snapshot(primary) : Promise.resolve(null), fetchArticles()]).then(function (r) {
      if (my !== token) return;
      var news = r[0] || [], snaps = {}; if (primary) snaps[primary] = r[1]; var articles = r[2] || [];
      mount(buildUserHTML(f, {}, snaps, news, {}, null, articles));
      ...
      Promise.all([
        ...
        primary ? D.liveTable(primary) : Promise.resolve(null),
          ...
          mount(buildUserHTML(f, matches, snaps, news, tables, null, articles));
          ...
          if (favM && favM.id) {
            D.liveMatch(fav.league_slug, favM.id).then(function (lm) {
              ...
              mount(buildUserHTML(f, matches, snaps, news, tables, lm, articles));
            });
          }
        }).catch(fail);
    }).catch(fail);
